...
global vjti_logo 
vjti_logo = Image.open('veermata-jijabai-technological-institute-mumbai-logo.png')
...
global login_logo
login_logo_img = Image.open('veermata-jijabai-technological-institute-mumbai-logo.png')
...
def resultPage():
    ...
    if score >= 8:
        result_img = Image.open('happy.png')
        ...
        result_img = Image.open('poor.png')
        result_img = ImageTk.PhotoImage(result_img)
        result_image.configure(image = result_img)
        result_image.image = result_img
    ...
    cursor.execute(sql)
    rec = cursor.fetchall()
    print(tabulate(rec,['Roll No.','Name','Branch'],tablefmt = 'psql'))

    sql = "SELECT * from Scores"
    cursor = connection.cursor()
    cursor.execute(sql)
    rec = cursor.fetchall()
    print(tabulate(rec,['Roll No.','Score'],tablefmt = 'psql'))
...
start_test_img  = Image.open('starttest-button.png')
...
sub_img = Image.open('SUBMIT-BUTTON.png')
